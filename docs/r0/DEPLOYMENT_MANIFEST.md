...
| R0 source branches | Merged through PIP PR #2 and FEA PR #10 |
| Runtime release SHA | `TBD_RECONCILIATION` |
| Runtime release artifact hash | `TBD_RECONCILIATION` |
| Directory-index hardening merge SHA | `c0306d6e0ad057a84cabebccb06c4069ea10ffe6` |
| Deployed `.htaccess` SHA-256 | `69066A0A0B84F1AE8C7806D33422C03B88B4616347CC4361C9E89F36349FE1A8` |
| Authenticated health evidence | GitHub Actions run `31328315903`, HTTP `200`, body not captured |
...
- `health.php` and `fixture.php` return HTTP `401` without credentials.
- Protected authenticated `health.php` validation returns HTTP `200` through the `pip-authenticated-readonly-validation` GitHub environment.
- Runtime files observed on the host predate the R0 merge, so source merge does not establish deployment identity.
- Remediation and evidence tracking: `nanotech-solutions-norway/Probability_Intelligence_Platform#3`.
